"""PDF extraction and chunking for Mutant: Year Zero rulebooks."""
...
# Regex to detect section headings (uppercase lines or lines ending with ':')
...
def extract_and_chunk(book_id: str, sources_path: str, chunk_size: int = 12000, overlap: int = 1500) -> list[PdfChunk]:
    ...
    for page_num in range(len(doc)):
        ...
        if not text.strip():
            # Possibly a scanned page — try OCR if pytesseract available
            text = _ocr_page(page)
...
def _build_page_offsets(pages_text: list[tuple[int, str]]) -> list[tuple[int, int, int]]:
    """Return list of (page_num, char_start, char_end) for each page."""
    offsets = []
    pos = 0
    for page_num, text in pages_text:
        length = len(text) + 1  # +1 for the newline join
...
def _split_text(text: str, chunk_size: int, overlap: int) -> list[tuple[int, int]]:
    """Split text into (start, end) index pairs, preferring section boundaries."""
    chunks = []
    pos = 0
    length = len(text)
    ...
    while pos < length:
        ...
        # Try to break at a section heading before the hard limit
        if end < length:
...
def _last_heading_pos(text: str) -> int | None:
    """Return position of the last heading in text, or None."""
...
def _detect_section(text: str) -> str:
    """Return a best-guess section name from the start of a chunk."""
...
def _ocr_page(page) -> str:
    """Attempt OCR on a page image. Returns empty string if unavailable."""
    try:
        import pytesseract
