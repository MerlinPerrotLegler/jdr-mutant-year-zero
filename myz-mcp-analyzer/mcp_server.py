#!/usr/bin/env python3
"""MCP Server — Mutant: Année Zéro Rulebook Analyzer.
...
Exposes 6 tools for an LLM orchestrator to:
  ...
  3. analyze_chunk         — (LLM handles this; tool loads prompt + formats input)
...
VAULT_PATH = os.environ.get("VAULT_PATH", "./vault-myz")
PROMPT_PATH = os.environ.get("PROMPT_PATH", "./prompt-extraction-myz.md")
CHUNK_SIZE = int(os.environ.get("CHUNK_SIZE", "12000"))
CHUNK_OVERLAP = int(os.environ.get("CHUNK_OVERLAP", "1500"))

# In-memory chunk cache (chunk_id → PdfChunk)
_chunk_cache: dict[str, pdf_handler.PdfChunk] = {}

server = Server("myz-analyzer")
    ...
    return [
        ...
        types.Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "chunk_id": {"type": "string", "description": "ID du chunk (ex: myz-core_p001_c001)"},
...
def _extract_and_chunk_pdf(args: dict) -> list[types.TextContent]:
    ...
    try:
        ...
        # Register book in progress tracker
